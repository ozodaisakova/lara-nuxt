...
<v-dialog 
    ...
    <v-divider></v-divider>
    <v-card-text>
        Вы действительно хотите удалить информационную страницу "<b>{{deletedItem.name}}</b>" ?
    </v-card-text>
    <v-divider></v-divider>
    <v-card-actions>
        <v-spacer></v-spacer>
    ...
    methods:{
        getCategories(){
            this.$axios.$get(this.$store.getters.base_url+"information")
                .then(response => {
                    this.categories=response.informations;
        ...
        destroyItem(){
            ...
            this.$axios.$delete(this.$store.getters.base_url+'information/'+id)
                .then(response=>{
