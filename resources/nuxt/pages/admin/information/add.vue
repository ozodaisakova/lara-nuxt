            ...
            class="my-4">
                <v-flex xs12 sm12 md4>          
                    <v-text-field
                        ...
                        :rules="nameRules"
                        counter="15"
                        hint="Напишите наименование страницы "
                        persistent-hint>
                    </v-text-field>                 
                </v-flex>
                <v-flex xs12 sm12 md4 class=" px-5">
                    <span class="">Выберите иконку</span>
...
<v-dialog
    ...
    <v-card>
        ...
        <v-divider></v-divider>
        <v-card-text>
            <h1 class="display-1 font-weight-bold text-xs-center">Ошибка {{errorStatus}}</h1>
            <p  class="title text-xs-center">{{errorText}}</p>
        </v-card-text>
        <v-divider></v-divider>
    ...
    data(){
        return{
            ...
            nameRules: [
                ...
                v => (v && v.length <= 15)|| 'Длина не должен превышать 15 символов'
            ],  
            icon: "3d_rotation",
    ...
    methods:{
        save(){
            if(this.$refs.form.validate()){
                if(this.icon==null||this.editor==""){
                    this.errorStatus="Не заполненные поля";
                    this.errorText="Пожалуйста, заполните все поля "
                    this.errorDialog=true;
                    
                }else{
                    if(this.editHidden==true) this.editHiddenNumber=0;
                    else this.editHiddenNumber=1;
                    
                    this.$axios.$post(this.$store.state.base_url+"information",
                    {
                        name: this.name,
                        icon: this.icon,
                        content: this.editor,
                        hidden: this.editHiddenNumber
                    })
                    .then(response=>{
                        this.snackbar=true;
                    })
